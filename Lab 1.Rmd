...
gdp_df$Country <- gsub("\\s*\\*$", "" , gdp_df$Country)


#gdp_df$Country <- gsub("\\s*\\*$", "", gsub("\\s+$", "", gdp_df$Country))
...
incarceration_df$Country <- gsub("\\s*\\*(\\s*\\[Note\\])?$", "", gsub("\\s+$", "", incarceration_df$Country))
incarceration_df$Country <- gsub("(\\s*\\[Note\\])?$", "", gsub("\\s+$", "", incarceration_df$Country))
...
joined_table <- countries %>%
  left_join(gdp_df, by = c("Country" = "Country")) %>%
...
countries <- merge(countries, components_lst, by = "Country", x.all = FALSE)

#Display the top 5 rows:
head(components_lst, 2)
