...
From the density plots above we can see that they d'ont resamble to the normal distribution In addition we can see how the Skewness reflect in the plots, where Middle East and North Africa and Sub-Saharan Africa have the right tail as a positive Skewness. er can see that the plots of the regions apart from North America have a negative Skewness. 
North America have a low variance and have no tail at all. It have Kurtosis of 1.
...
We can see that all the regions did not had any significant changes between 2006 to 2022. most of them keep staing at the same values.
3.b.
...
```{r, cache=TRUE}
dem_gdp <- joined_table %>% select(dem_index = X2022, cia_gdp = `CIA.8..9..10. - Estimate`, incarceration = Rate.per.100.000..3.)# %>% filter(! is.na(dem_index) & ! is.na(cia_gdp) )  #
...
The first plot of linear regression  (the blue dots) - GDP explained by democracy index. We can some linear connection between the variables and we have few high GDP with respectively low democracy index.
The second plot (the red dots) - Incarceration explained bt democracy index. we can see there is no corrolation between this two.
...
We can see here the differences between X plot(6.a) to Y plot (6.b), where as Y reflect more accurately the GDP around the world - 90% of people are below 30,000 GDP per year, very low productivity.
...
At the first heat map of the world we can see that the western world is more democratic then the eastern side ( except Australia and most of Europe).
The second heat map show the difference index between 2022 to 2006. we can see that the south world became more democratic. 
...
Another thing that can affect GDP is that a country have natural resources like Qatar, United Arab Emirates and Luxembourg.
