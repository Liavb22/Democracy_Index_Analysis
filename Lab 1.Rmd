...
## we need to check if there is a problem with the last varible - cia_gdp = `CIA.8..9..10. - Estimate` do we need it for the metrix??

#cor_data <- total_data %>% select(Electoral_process_and_pluralism, Functioning_of_government, Political_participation, Political_culture, Civil_liberties,cia_gdp = `CIA.8..9..10. - Estimate`) %>% filter(! is.na(Civil_liberties))
...
```{r, cache=TRUE}
democracy_gdp_model <- lm(formula = cia_gdp ~ Electoral_process_and_pluralism + Functioning_of_government + Political_participation + Political_culture + Civil_liberties, data = cor_data)
...
The coefficient that are significat at s.l of o.o1 are Electoral_process_and_pluralism, Functioning_of_government and Political_culture
